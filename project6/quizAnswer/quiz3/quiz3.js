    ...
    function validate(f){
        ...
        // 테이블에 회원을 추가
        addDataToTable(); // addDataToTable(); 함수를 이용하여 테이블에 추가
    ...
    function addDataToTable() {
        // 폼 데이터를 가져옴
        const id = document.getElementById('id').value; // value 속성
        const name = document.getElementById('name').value;
        // name이 "gender"인 입력필드 중에서, 사용자가 선택한 요소 찾기
        // 필드선택자 + 속성선택자 + 상태선택자
        const gender = document.querySelector('input[name="gender"]:checked'); // 성별 , ('태크[00속성]: 상태(체크된 것만 가져온다)')
        const interests = document.querySelectorAll('input[name="interest"]:checked'); // 취미
