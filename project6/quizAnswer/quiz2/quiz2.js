...
function validateAndAddItem() {
  ...
  // 유효성 체크에 실패했으면 함수 종료
  // 위에서 하나라도 비어있다면(조건에 맞족하지 못했다면) 이쪽으로 이동
    if (!isValid) {
        alert(errorMessage);
        return; // 강제 함수 종료, 밑에 있는 함수를 만날수 없다.
    }
...
function addItemToCart(name, quantity, price) {
    ...
    if (existingItem) {
        ...
        var newPrice = (parseFloat(existingPrice.textContent) + (quantity * price)).toFixed(2); //원본데이터 + 새로추가된 데이터 값

        existingQuantity.textContent = newQuantity;
        existingPrice.textContent = newPrice;
    } else {

        // let  newItem = cartList.insertAdjacentHTML("beforeend",`<li data-name="${name}">${name} - <span class="itemQuantity">${quantity}</span>개<span>${quantity*price}</span>원<button onclick="removeItem(this)">제거</button></li>`);
        ...
        // cartList.insertAdjacentHTML("beforeend", "<li>새로운항목<li>");
    }
