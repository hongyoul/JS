...
// 2) 만약 학생이 해당 과목의 점수를 가지고 있지 않으면 계산하지 않고, 해당 학생은 건너뜁니다.
...
function average(name, ...score) {
  let total = 0;
  // 배열을 순회하며 합계 구하기
  for (let x of score) {
    total += x;
  }
  const student = { name: "짱구", eng: 80, math: 50, kor: 70 };
  console.log(total/3); // 합계
}

  console.log("짱구", 80, 50, 70);
